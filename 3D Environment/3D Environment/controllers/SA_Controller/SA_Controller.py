...
class FrontierGoalSelector:
    """
    Stochastic selector over utility-scored candidates.
    Lower score is better.
    """

    def __init__(self, seed=7, T0=2.5, Tmin=0.25, cooling=0.997):
    ...
    def pick(self, candidates):
        ...
        candidates = candidates[:120] # Keep only the best few candidates, then sample among them.

        best_score = candidates[0][0]
        weights = []
        for score, cell in candidates:
            w = math.exp(-(score - best_score) / max(self.T, 1e-6))
            weights.append(w)
        ...
        if s <= 0.0:
            ...
            cum = 0.0
            goal = candidates[0][1]
            for (_, cell), w in zip(candidates, weights):
                cum += w / s
                if r <= cum:
                    goal = cell
                    break

        self.T = max(self.Tmin, self.T * self.cool) # Gradually reduce randomness so selection becomes greedier over time.
        return goal


class MavicSAController:
    def __init__(self):
        self.robot = Robot()
        ...
        self.selector = FrontierGoalSelector(seed=7)
    ...
    # Convert world coordinates into grid indices without clamping to bounds.
    ...
    # Mark all cells inside the sensor reveal radius as visited.
    ...
    def frontier_neighbor_count(self, r, c):
        cnt = 0
        for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            nr, nc = r + dr, c + dc
            if self.cell_in_region(nr, nc) and (nr, nc) in self.visited:
                cnt += 1
        return cnt
        
    # A frontier is an unvisited cell adjacent to at least one visited cell.
    def is_frontier(self, r, c):
        if (r, c) in self.visited:
            return False
        return self.frontier_neighbor_count(r, c) > 0

    # Estimate how much new area would be revealed if this candidate were chosen.
    def candidate_information_gain(self, cell):
        r0, c0 = cell
        gx, gy = self.cell_to_world_center(r0, c0)
        ...
        for dr in range(-rad_cells, rad_cells + 1):
            for dc in range(-rad_cells, rad_cells + 1):
                ...
                if not self.cell_in_region(r, c):
                    continue
                cx, cy = self.cell_to_world_center(r, c)
                if (cx - gx) ** 2 + (cy - gy) ** 2 <= rad2:
                    if (r, c) not in self.visited:
                        gain += 1
        return gain
    
    # Score balances distance, information gain, revisit penalty, and edge penalty.
    def candidate_score(self, cur_cell, cand_cell):
        cr, cc = cur_cell
        r, c = cand_cell

        dist = abs(r - cr) + abs(c - cc)
        gain = self.candidate_information_gain(cand_cell)

        edge_d = min(r, c, self.grid_H - 1 - r, self.grid_W - 1 - c)
        edge_penalty = 0.0
        if edge_d < self.boundary_hard_margin_cells + 1:
            edge_penalty = self.score_edge_w * (self.boundary_hard_margin_cells + 1 - edge_d)

        revisit_penalty = self.score_revisit_w * self.visit_counts.get((r, c), 0)
        frontier_bonus = self.frontier_neighbor_bonus * self.frontier_neighbor_count(r, c)

        score = (
            self.score_dist_w * dist
            - self.score_gain_w * gain
            + edge_penalty
            + revisit_penalty
            - frontier_bonus
        )
    ...
    def build_candidates(self, cur_cell):
        ...
        for r in range(H):
            for c in range(W):
                ...
                if self.is_frontier(r, c):
                    frontier_cells.append((r, c))

        pool = frontier_cells if frontier_cells else unvisited_cells

        candidates = []
        for cell in pool:
            candidates.append((self.candidate_score(cur_cell, cell), cell))

        candidates.sort(key=lambda x: x[0])
        return candidates[: self.max_candidate_eval]
    ...
    def choose_new_goal(self, x, y):
        ...
        goal_cell = self.selector.pick(candidates)
        if goal_cell is None:
            return None
        return self.cell_to_world_center(goal_cell[0], goal_cell[1])
    
    # Compute desired roll/pitch commands from goal direction and current body velocity.
    ...
    # Push the drone back toward the interior when it gets too close to the boundary.
    ...
    # Draw explored cells, trail, current position, and active goal.
    ...
    # Low-pass filter altitude and estimate vertical speed from GPS altitude.
    ...
    # Add slow altitude variation to avoid flying at exactly one height forever.
    ...
    # Prefer facing the current travel direction; if slow, face the goal instead.
    ...
    # Return the minimum valid lidar range inside a normalized sector.
    ...
    # ---------- Main ----------
    def run(self):
        print("Starting frontier / information-gain controller with OA-guided motion.")
        print(
        ...
        # Wait briefly so all sensors provide stable initial readings.
        ...
        try:
            while self.robot.step(self.timestep) != -1:
                ...
                # Initialize the local coverage frame using the drone's starting position.
                if self.origin_xy is None:
                    self.origin_xy = (x, y)
                ...
                # Transition from motor spool-up to active takeoff.
                ...
                # Enable coverage mode once the drone has reached a safe airborne altitude.
                ...
                # Panic mode suppresses aggressive motion and prioritizes recovering altitude.
                ...
                # Pick or update a coverage goal only when lateral motion is safe.
                    ...
                    if self.goal_world is not None:
                        ...
                        if self.goal_prev_dist is None:
                            ...
                            if dist < self.goal_prev_dist - self.goal_progress_epsilon_m:
                                ...
                                # If progress toward the current goal stalls for too long, replan.
                ...
                # Boundary recovery overrides normal coverage behavior near or outside the map.
                ...
                # Obstacle avoidance temporarily replaces normal goal-following commands.
                ...
                # Final motor commands combine altitude, attitude stabilization, navigation, and yaw.
            ...
            if self.metrics_started:
                self._final_print() # Print final coverage and path metrics


if __name__ == "__main__":
    MavicSAController().run()