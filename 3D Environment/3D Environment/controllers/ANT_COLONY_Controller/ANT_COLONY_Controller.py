...
# Normalize an angle to [-pi, pi] so heading errors stay bounded.
def wrap_pi(a):
    while a > math.pi:
        a -= 2.0 * math.pi
    while a < -math.pi:
...
class ACOGoalSelector:
    """
    Pure ACO selector:
    - pheromone per cell
    - heuristic = inverse cell distance
    - stochastic selection from tau^alpha * eta^beta
    - evaporation + reward deposit after finishing a goal
    """
    ...
    def __init__(
        ...
        # Initialize one pheromone value per grid cell.
        self.pher = [[float(tau0) for _ in range(W)] for _ in range(H)]

    # Apply uniform pheromone evaporation across the grid.
    def _evaporate(self):
        k = 1.0 - self.rho
    ...
    # Deposit pheromone on the completed goal in proportion to newly revealed cells.
    def deposit(self, goal_cell, newly_revealed):
        ...
        if newly_revealed > 0:
            v = self.pher[gr][gc] + self.reward * float(newly_revealed)
            self.pher[gr][gc] = min(self.tau_max, max(self.tau_min, v))
    
    # Rank candidates by pheromone strength and inverse distance, then sample stochastically.
    def pick(self, candidates):
        if not candidates:
            return None

        candidates.sort(key=lambda x: x[0])
        # Restrict sampling to the strongest candidate subset for efficiency.
        ...
        for d, (r, c) in candidates:
            ...
            tau = self.pher[r][c]
            eta = 1.0 / float(d)
            # ACO selection rule: tau^alpha * eta^beta.
            numer.append((tau ** self.alpha) * (eta ** self.beta))
        ...
        cum = 0.0
        goal = candidates[0][1]
        for (_, cell), val in zip(candidates, numer):
            cum += val / s
            if rnum <= cum:
                goal = cell
                break
...
class MavicACOController:
    def __init__(self):
        ...
        # ACO bookkeeping for reward
        self._visited_at_goal_start = 0
    ...
    # Convert world coordinates into grid indices without clipping to the map bounds.
    ...
    # ---------- Pure ACO candidate generation ----------
    
    # Build the ACO candidate list using all currently unvisited cells.
    def build_candidates(self, cur_cell):
        cr, cc = cur_cell
        H, W = self.region_dims()
        candidates = []
        for r in range(H):
    ...
    # Reward the finished goal based on how much new area was revealed while pursuing it.
    def _finish_goal_and_update_pheromone(self, finished_cell):
        if finished_cell is None:
            return
        newly = max(0, len(self.visited) - int(self._visited_at_goal_start))
        self.aco.deposit(finished_cell, newly)
    
    # Choose the next goal cell using the ACO selector and store the current visit count baseline.
    def choose_new_goal(self, x, y):
        cur_cell = self.world_to_cell(x, y)
        self._mark_cell_visit(cur_cell)

        candidates = self.build_candidates(cur_cell)
        goal_cell = self.aco.pick(candidates)
        if goal_cell is None:
            return None, None

        goal_world = self.cell_to_world_center(goal_cell[0], goal_cell[1])
        self._visited_at_goal_start = len(self.visited)
        return goal_world, goal_cell

    # Convert goal direction into desired roll/pitch motion in the drone body frame.
    ...
    # Push the drone back toward the valid coverage area when it nears the boundary.
    ...
    # Draw explored cells, path trail, current position, and active goal.
    ...
    # Low-pass filter altitude and estimate vertical speed from successive altitude readings.
    ...
    # Add slow vertical variation to reduce staying at one fixed altitude.
    ...
    # Prefer facing the travel direction; if moving slowly, face the goal instead.
    ...
    # Estimate body-frame velocity from GPS position changes and current yaw.
    ...
    # Track path length, revisits, and simple energy usage during the run.
    ...
    def _final_print(self): # Print final coverage and motion metrics,
        H, W = self.region_dims()
        total_cells = H * W
        cov = 100.0 * len(self.visited) / max(1, total_cells)
    ...
    # Return the minimum valid lidar range within a normalized angular sector.
    ...
    # Generate avoidance behavior when obstacles block forward motion.
    ...
    def run(self):
        ...
        # Let the simulation settle briefly before starting control.
        ...
        try:
            while self.robot.step(self.timestep) != -1:
                ...
                # Use the initial position as the local origin for the coverage map.
                if self.origin_xy is None:
                    self.origin_xy = (x, y)
                ...
                # Ramp the motors gradually before switching into takeoff mode.
                ...
                # Enter coverage mode once a safe airborne altitude has been reached.
                ...
                # Panic mode suppresses aggressive lateral motion and prioritizes altitude recovery.
                panic_mode = has_airborne and (
                    (altitude < self.panic_alt)
                    or (altitude < (floor_alt + 0.8) and vz < -0.8)
                )
                ...
                if has_airborne and self.origin_xy is not None:
                    ...
                    if out_of_bounds:
                        ...
                        if self.goal_cell is not None:
                            self._finish_goal_and_update_pheromone(self.goal_cell)
                ...
                # Choose or refresh the ACO goal only when normal lateral motion is allowed.
                if enable_lateral and (not self.oa_detour_mode):
                     # Finish the previous goal and deposit pheromone before selecting a new one.
                    if self.goal_world is None or self.goal_hold_steps > self.max_goal_hold_steps:
                        if self.goal_cell is not None:
                            self._finish_goal_and_update_pheromone(self.goal_cell)

                        self.goal_world, self.goal_cell = self.choose_new_goal(x, y)
                        self.goal_hold_steps = 0
                        self.goal_blocked_time = 0.0
                        self.goal_prev_dist = None
                        self.goal_progress_timer = 0.0
                    ...
                    if self.goal_world is not None:
                        ...
                        if self.goal_prev_dist is None:
                            ...
                            if dist < self.goal_prev_dist - self.goal_progress_epsilon_m:
                                ...
                                # If the drone stops making progress toward the goal, abandon it and replan.
                ...
                # Boundary recovery overrides normal planning when the drone gets too close to the edge.
                ...
                # Near the floor, reduce tilt authority and cancel the current planning goal.
                if near_floor:
                    if self.goal_cell is not None:
                        self._finish_goal_and_update_pheromone(self.goal_cell)
                    self.goal_world = None
                ...
                # Obstacle avoidance temporarily replaces normal ACO goal-following motion.
                ...
                # Combine stabilization, navigation, altitude, and yaw into final motor commands.
                ...
                if t - self._last_debug_t > 1.0:
                    ...
                    print(
                        ...
                        f"t={self._sim_elapsed():.0f}s goal={gtxt}"
                    )
